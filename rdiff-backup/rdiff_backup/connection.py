...
import types, os, tempfile, six.moves.cPickle, shutil, traceback, \
	   socket, sys, gzip
...
class LowLevelPipeConnection(Connection):
	...
	def _putobj(self, obj, req_num):
		...
		self._write("o", six.moves.cPickle.dumps(obj, 1), req_num)
	...
	def _putrpath(self, rpath, req_num):
		...
		self._write("R", six.moves.cPickle.dumps(rpath_repr, 1), req_num)
	...
	def _putqrpath(self, qrpath, req_num):
		...
		self._write("Q", six.moves.cPickle.dumps(qrpath_repr, 1), req_num)
	...
	def _putrorpath(self, rorpath, req_num):
		...
		self._write("r", six.moves.cPickle.dumps(rorpath_repr, 1), req_num)
	...
	def _get(self):
		...
		if format_string == "o": result = six.moves.cPickle.loads(data)
	...
	def _getrorpath(self, raw_rorpath_buf):
		...
		index, data = six.moves.cPickle.loads(raw_rorpath_buf)
		return rpath.RORPath(index, data)

	def _getrpath(self, raw_rpath_buf):
		"""Return RPath object indicated by raw_rpath_buf"""
		conn_number, base, index, data = six.moves.cPickle.loads(raw_rpath_buf)
	...
	def _getqrpath(self, raw_qrpath_buf):
		...
		conn_number, base, index, data = six.moves.cPickle.loads(raw_qrpath_buf)
