...
class ExtendedAttributes:
	...
	def read_from_rp(self, rp):
		...
		for attr in attr_list:
			...
			try:
				...
				elif exc[0] == errno.E2BIG: continue
				else: raise
	...
	def write_to_rp(self, rp):
		...
		self.clear_rp(rp)
		for (name, value) in six.iteritems(self.attr_dict):
			try:
...
def EA2Record(ea):
	...
	for (name, val) in six.iteritems(ea.attr_dict):
...
def Record2EA(record):
	...
	if filename == '.': index = ()
		...
		if Globals.use_unicode_paths:
			unquoted_filename = six.text_type(unquoted_filename, 'utf-8')
