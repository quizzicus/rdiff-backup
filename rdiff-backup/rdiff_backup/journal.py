...
import Globals, log, rpath, six.moves.cPickle, TempFile, os, restore
...
class Entry:
	...
	def to_string(self):
		...
		return six.moves.cPickle.dumps({'index': self.index,
	...
	def init_from_string(self, s):
		...
		try: val_dict = six.moves.cPickle.loads(s)
		except six.moves.cPickle.UnpicklingError: return 0
		try:
